...
class Product(models.Model):
    ...
    title_short = models.CharField(max_length=200, verbose_name='عنوان کوتاه فارسی')
    ...
    offer = models.BooleanField(default=False, verbose_name='پیشنهاد امروز')
    expire_date = models.DateTimeField(blank=True, verbose_name='تاریخ انقضا محصول')
    ...
    def get_avg_stars_percent(self):
        product_stars = Star.objects.filter(product=self)
        product_stars_scores = [bs.score for bs in product_stars]
        if len(product_stars) == 0:
            avg = 0
        else:
            avg = sum(product_stars_scores) / len(product_stars)
        return int((avg/5)*100)
    
    @property
    def product_stars_number(self):
        product_stars = Star.objects.filter(product=self)
        return len(product_stars)
...
class Star(models.Model):
    ...
    product = models.ForeignKey(Product, on_delete=models.CASCADE, related_name='stars', verbose_name='محصول')
    class Meta:
        verbose_name = 'امتیاز ستاره‌ای'
        verbose_name_plural = 'امتیازهای ستاره‌ای'
    def __str__(self):
        return '{} - {} - {}'.format(self.score, self.product.name, self.user.username)