...
def home_page(request):
    ...
    category = Category.objects.filter(status=True).filter(parent=None)
    product_all_sale = Product.objects.filter(category__in=Category.objects.get(slug='corona') \
    ...
    day_offer = Product.objects.filter(status=True).filter(offer=True)
    context = {
        'day_offer': day_offer,
        'category': category,
        'sliders': sliders,
