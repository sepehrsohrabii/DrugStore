...
from eshop_order.forms import ShopCartForm, PostWayForm, PayWayForm, CouponForm
...
def shopcart(request):
    ...
    if coupon_form.is_valid():
        ...
        status = validate_coupon(coupon_code=coupon_code, user=current_user)
        if status['valid']:
            coupon = Coupon.objects.get(code=coupon_code)
            #coupon.use_coupon(user=current_user)
    ...
    for rs in shopcart:
        ...
        if rs.product.variant == 'None' or variants.count() == 0:
            ...
            #totalPrice_takhfif = int(totalPrice_takhfif * ((100 - discount_value) * 0.01))
            #totalTakhfif = totalPrice - totalPrice_takhfif
        else:
            totalPrice += rs.variant.price * rs.quantity
            totalPrice_takhfif += rs.variant.get_product_offer_price * rs.quantity
            totalCount += rs.quantity
            #totalPrice_takhfif = totalPrice_takhfif * ((100 - discount_value) * 0.01)
            #totalTakhfif = totalPrice - totalPrice_takhfif

    totalPrice_takhfif_withCoupon = int(totalPrice_takhfif * ((100 - discount_value) * 0.01))
    totalTakhfif = totalPrice - totalPrice_takhfif_withCoupon
    coupon_takhfif = totalPrice_takhfif - totalPrice_takhfif_withCoupon
    ...
    if request.method == 'POST':  # if there is a post
        ...
        data.total_products = totalPrice_takhfif
        data.total_products_takhfif = totalPrice_takhfif_withCoupon
        data.total = totalPrice_takhfif_withCoupon

        if coupon_form.is_valid() :
            data.coupon = coupon_code
            data.coupon_takhfif = coupon_takhfif
    ...
    context = {'shopcart': shopcart,
               ...
               'totalPrice_takhfif': totalPrice_takhfif,
               'totalTakhfif': totalTakhfif,
               'totalPrice_takhfif_withCoupon': totalPrice_takhfif_withCoupon,
...
def shipping_page(request):
    ...
    for rs in shopcart:
        ...
        if rs.product.variant == 'None' or variants.count() == 0:
            totalPrice += rs.product.price * rs.quantity
            totalPrice_takhfif += rs.product.get_product_offer_price * rs.quantity
            if selected_way.price == None:
                totalPriceWithPost = totalPrice
                totalPrice_takhfifWithPost = totalPrice_takhfif
            else:
                totalPriceWithPost = totalPrice + (selected_way.price)
                totalPrice_takhfifWithPost = totalPrice_takhfif + (selected_way.price)
            totalCount += rs.quantity
            totalTakhfif = totalPrice - totalPrice_takhfif
        else:
            totalPrice += rs.variant.price * rs.quantity
    ...
    context = {
        ...
        'totalPrice_takhfif': totalPrice_takhfif,
        'totalTakhfif': totalTakhfif,
        'totalPrice_takhfifWithPost': totalPrice_takhfifWithPost,
...
def order_completed(request):
    ...
    if request.method == 'POST':  # if there is a post
        ...
        data.total_products = totalPrice_takhfif
        #data.total_products_takhfif = 

        data.total = totalPrice_takhfifWithPost
        data.amount = totalCount
        data.ip = request.META.get('REMOTE_ADDR')
        ordercode = get_random_string(10).upper()  # random cod
        data.code = ordercode
